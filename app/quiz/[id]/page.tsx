...
// Platform-level quiz player. Most students reach the quiz through the
// tenant-scoped route /p/[tenant]/quiz/[id]. This page is a fallback
// for links shared without tenant context. Signed-in users are bounced
// to the tenant route. Anonymous users see a guest entry form first.

import { use, useEffect, useState } from "react"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { ArrowLeft, FileQuestion, GraduationCap, Loader2, Mail, User } from "lucide-react"
...
export default function PublicQuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch from server if not in localStorage (guest on another device)
  ...
  // Bounce signed-in students to the tenant-scoped route
  useEffect(() => {
